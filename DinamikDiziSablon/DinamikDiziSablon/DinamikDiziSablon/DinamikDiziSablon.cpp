...
public:
	DinamikDizi() {
		m_boyut = 5;
		m_adres = new Tur[m_boyut];
		m_kullanilan = 0;
	}
	~DinamikDizi() {
		delete[] m_adres;
	}
	const Tur& getir(int sira) const {
	...
	void sonaEkle(const Tur& yeniSayi)
	{
	...
	void genislet(int ekBoyut)
	{
		if (ekBoyut >= m_boyut) 
		{
			Tur* yeniAdres = new Tur[m_boyut + ekBoyut];
		...
		{
			throw exception("genislet():arguman hatasi");
		}
	}
	void arayaEkle(int sira, const Tur& veri)
	{
		if (sira >= 0 && sira <= m_kullanilan)
		{
...
public:
	...
	Sayi(const Sayi& sag)
	{
		cout << "Sayi(const Sayi& sag)" << endl;
	}
};
int main()
{
	DinamikDizi<Sayi> dizi;
	cout << "---" << endl;
